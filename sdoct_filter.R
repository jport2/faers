...
sdoct$drugoutcome <- paste0(sdoct$X2.y, sdoct$X2.x)
ryan <- read.csv("cleanRyan.csv", header=FALSE) 
...
ryan$drugoutcome <- paste0(ryan$V1,ryan$V2)
sdoct$drugoutcome <- str_replace_all(sdoct$drugoutcome," ","_")
sdoct$drugoutcome <- as.factor(as.character(sdoct$drugoutcome))
ryan$drugoutcome <- as.factor(as.character(ryan$drugoutcome))

merge <- left_join(sdoct, ryan, by="drugoutcome")
merge <- merge[complete.cases(merge$V3),]
dim(merge)

setdiff(ryan$drugoutcome, merge$drugoutcome)

# basicplot <- ggplot(merge, aes(d = Ground.Truth..1.postive.control..0.negative.control., m = prr)) + geom_roc()
library(pROC)
plot <-plot.roc(merge$V3, merge$prr, type="b", pch=21, col="blue", bg="grey")
plot
