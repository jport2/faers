...
m = input3dep.shape[1]*i + j
df['Drug'].iloc[m] = input2.columns[i]
df.ix[m,"Drug"] = input2.columns[i]
df.ix[m,"Outcome"] = output2.columns[j]
df.ix[m,"Score"]= sigscalar
df.ix[,j:,2] = output2.columns[j]
df.ix[,j:,3] = sigscalar

# paste function that doesn't work
import functools
