# FAERS NORMALIZED (BANDA) MERGED FOR DATA SET CREATION FOR PS AND SS
...
ps <- drugoutcome %>% filter(role_cod == "PS" | role_cod == "SS")
psswitch <- ps
banda <- drugoutcome
...
setwd("/home/data/jportanova/FAERS/ascii/psssonlybanda")
write.table(ps,file="psss_only_standardized.txt", row.names=FALSE, quote=FALSE, sep=" ")
...
setwd("/home/data/jportanova/FAERS/ascii/psssswitchbanda")
write.table(psswitch2,file="psssswitch_only_standardized.txt", row.names=FALSE, quote=FALSE, sep=" ")
